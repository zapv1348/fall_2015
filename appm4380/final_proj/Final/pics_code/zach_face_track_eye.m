...
faceDetector = vision.CascadeObjectDetector('RightEyeCART');
...
figure, imshow(videoFrame), hold on, title('Detected features 2');
plot(points);
...
while ~isDone(videoFileReader)
    % get the next frame
    videoFrame = step(videoFileReader);
    if size(visiblePoints, 1) < l-10 %reget points
    %if n>40
        bbox            = step(faceDetector, videoFrame);
        bboxPoints = bbox2points(bbox(1, :));
        points = detectMinEigenFeatures(rgb2gray(videoFrame), 'ROI', bbox(1,:));
        second=points;
        second2=second.Location;
        points =points.Location;
