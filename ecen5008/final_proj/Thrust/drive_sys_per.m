% drive_sys_per - play with a system ... check proj operator, etc.
%
%   example:
%
%   play with the 
...
% our 'play example' will use a periodic (v,alat)(.) curve
Tper = 10;
Nper = 5;
Wper = 2*pi/Tper;
...
% set up a function space: choose a discretation level & final time
dt = 0.01;
% t1 = 10.0;  % defined above
T0 = (0:dt:t1)';
...
Alat_des  = alat_max*sin(Wper*T0);
dAlat_des = alat_max*cos(Wper*T0)*Wper;


v0 = 10;
delv = 2;

V_des  = v0 + delv*cos(Wper*T0);
dV_des =    - delv*sin(Wper*T0)*Wper;

Beta_des = beta_alat * Alat_des;

Omega_des  = Alat_des ./ V_des;
dOmega_des = (dAlat_des.*V_des - Alat_des.*dV_des) ./ V_des.^2;

U1_des = u1_alat2 * Alat_des.^2/2;   %maybe: + dV_des./cos(Beta_des);
U1_des = u1_alat2 * Alat_des.^2/2 + dV_des./cos(Beta_des);
...
% as defined above, we expect
%   x00 = [ v0 0 0 ], u00 = [ 0 0 ]
x00 = Xdes(1,:);
u00 = Udes(1,:);


% let's get the linearization at (x00,u00)
%
%   use:  doc dynamics_m
